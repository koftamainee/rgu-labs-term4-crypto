...
namespace math {
  ...
  public:
    ...
    virtual bool is_prime(const bigint& n, double min_probability) = 0;
  };

  class PrimeTest : public IPrimeTest {
  public:
    bool is_prime(const bigint& n, double min_probability) final;

  protected:
    virtual int calculate_iterations(double min_probability) const;

    virtual bool single_test_iteration(const bigint& n, int iteration_index) const = 0;
